...
let test_success_cases () =
  let test_cases = [
    ...
    ("Multiple variables", "fn main() { let a = 1; let b = 2; let c = a + b; }");
    ("Built-in functions", "fn main() { let s = int_to_string(42); print(s); }");
...
let test_error_cases () =
  let test_cases = [
    ("Undefined variable", "fn main() { let x = undefined_var; }", "Undefined variable");
    ("Type mismatch in arithmetic", "fn main() { let x = true + 10; }", "Type mismatch in binary operation");
    ("Undefined function", "fn main() { unknown_func(); }", "Undefined function");
    ("Wrong argument type", "fn main() { print(42); }", "expects different argument types");
    ("Wrong number of arguments", "fn main() { print(\"hello\", \"world\"); }", "wrong number of arguments");
    ("Boolean in arithmetic", "fn main() { let x = false * 5; }", "Type mismatch in binary operation");
    ("String in arithmetic", "fn main() { let x = \"hello\" + 5; }", "Type mismatch in binary operation");
    ("If condition not boolean", "fn main() { if 42 { let x = 1; } }", "If condition must be boolean");
  ] in
  
  Printf.printf "\n--- Type Checker Error Cases ---\n";
  List.iter (fun (name, code, expected_error) ->
    try
      let (lexer_fn, lexbuf) = Plato.Lexer.parse_string code in
      Lexing.set_filename lexbuf "test.plato";
      let ast = Plato.Parser.program lexer_fn lexbuf in
...
let test_complex_programs () =
  let test_cases = [
    ("Function with parameters", 
     "fn add(x: int, y: int) -> int { return x + y; } fn main() { let result = add(10, 20); }");
    ("Multiple functions", 
