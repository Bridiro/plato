...
let test_keywords () =
  ...
  List.iter
    (fun (word, expected) ->
      ...
      with exn ->
        Printf.printf "✗ Keyword '%s' failed: %s\n" word
...
let test_operators () =
  ...
  List.iter
    (fun (op, expected) ->
      ...
      with exn ->
        Printf.printf "✗ Operator '%s' failed: %s\n" op (Printexc.to_string exn))
    operators
...
let test_literals () =
  ...
  List.iter
    (fun (lit, expected) ->
      ...
      with exn ->
        Printf.printf "✗ Literal '%s' failed: %s\n" lit (Printexc.to_string exn))
    literals
...
let test_identifiers () =
  ...
  List.iter
    (fun (id, expected) ->
      ...
      with exn ->
        Printf.printf "✗ Identifier '%s' failed: %s\n" id
          (Printexc.to_string exn))
    identifiers
    ...
    (fun (expr, desc) ->
      try
        ...
        Printf.printf "✓ Expression '%s' (%s) -> %d tokens\n" expr desc
          token_count
      with exn ->
        Printf.printf "✗ Expression '%s' failed: %s\n" expr
          (Printexc.to_string exn))
    expressions
