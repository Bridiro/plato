...
let test_complete_programs () =
  let programs =
    [
      ...
      ( "struct Point { x: int, y: int } fn main() { let p = Point { x: 1, y: \
         2 }; }",
      ...
      ( "fn main() { if true { let x = 1; } else { let x = 2; } }",
...
let test_complex_features () =
  let complex_tests =
    [
      ( "fn factorial(n: int) -> int { if n <= 1 { 1 } else { n * factorial(n \
         - 1) } }",
        "Recursive function" );
      ( "struct Point { x: int, y: int } impl Point { fn new(x: int, y: int) \
         -> Point { Point { x, y } } }",
        "Struct with impl block" );
      ( "enum Result { Ok(int), Err(str) } fn test() -> Result { \
         Result::Ok(42) }",
