...
let type_error_at pos message =
  raise (Error.type_error ~filename:None ~line:pos.line ~column:pos.column ~offset:0 message)
...
let rec check_expression env = function
  ...
  | FunctionCall (func_expr, args, pos) ->
      let (param_types, return_type) = match func_expr with
  ...
  | If (cond, then_block, else_block_opt, pos) ->
      ...
      (match else_block_opt with
       | Some else_block ->
           ...
           else type_error_at pos "If branches have different types"
       | None -> TUnit)
  | Return (expr_opt, _) ->
      (match expr_opt with
       | Some expr -> check_expression env expr
            ...
            with Not_found -> 
              ...
              | Some global_fields ->
                  (try List.assoc field_name global_fields
                   with Not_found -> type_error_at pos ("Struct field not found: " ^ field_name))
  ...
  | Cast (expr, target_type, pos) ->
      ...
      let is_valid_cast = match source_norm, target_norm with
        | TI32, TI32 -> true | TI32, TF64 -> true | TF64, TI32 -> true
        | TI32, TStr -> true | TStr, TI32 -> true | TBool, TI32 -> true
        | TI32, TBool -> true | TPointer _, TI32 -> true | TI32, TPointer _ -> true
        | _ when source_norm = target_norm -> true
        | _ -> false in
      if is_valid_cast then target_simple
      else type_error_at pos "Invalid cast between types"
  | PathExpr (path, _pos) ->
      let name = String.concat "::" path in
       ...
       | first_type :: rest_types ->
           List.iter (fun arm_type ->
             if normalize_type arm_type <> normalize_type first_type then
               let pos = get_expression_position (match match_arms with 
                 | MatchArm (_, expr) :: _ -> expr | [] -> scrutinee) in
      ...
      (match expr_opt with
       ...
       | None -> 
           ...
           (match typ_opt with
            ...
            | None ->
                raise (Error.parse_error ~filename:None ~line:1 ~column:1 ~offset:0 "Variable declaration requires either type annotation or initializer")))
      ...
      let lvalue_type = match lvalue with
        | LvalueId name ->
            (match lookup_variable env name with
             | Some var_type -> var_type
             | None ->
                 raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 ("Undefined variable in assignment: " ^ name)))
        | LvalueDeref lval ->
            ...
            let ptr_type = match lval with
              | LvalueId name ->
                  (match lookup_variable env name with
                   ...
                   | Some _ ->
                       raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 "Cannot dereference non-pointer type")
                   | None ->
                       raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 ("Undefined variable in dereference: " ^ name)))
              | _ ->
                  raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 "Complex lvalue dereferencing not fully supported")
            in ptr_type
            ...
            let base_type = match lval with
              | LvalueId name ->
                  (match lookup_variable env name with
                   ...
                   | Some _ ->
                       raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 "Cannot index non-array type")
                   | None ->
                       raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 ("Undefined variable in index: " ^ name)))
              | _ ->
                  raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 "Complex lvalue indexing not fully supported")
            in
            let index_type = check_expression env index_expr in
            if normalize_type index_type <> TI32 then
              type_error_at (get_expression_position index_expr) "Array index must be integer";
            base_type
            ...
            let base_type = match lval with
              | LvalueId name ->
                  (match lookup_variable env name with
                   ...
                   | None ->
                       raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 ("Undefined variable in field access: " ^ name)))
              | _ ->
                  raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 "Complex lvalue field access not fully supported")
            in
             ...
             | TStruct (struct_name, fields) ->
                 (try List.assoc field_name fields
                  with Not_found ->
                    ...
                    | Some global_fields ->
                        (try List.assoc field_name global_fields
                         with Not_found -> raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 ("Struct field not found: " ^ field_name)))
                    | None -> raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 ("Struct field not found: " ^ field_name)))
             | _ -> raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 "Field access requires struct type"))
            ...
            let base_type = match lval with
              | LvalueId name ->
                  (match lookup_variable env name with
                   | Some (TPointer inner_type) -> inner_type
                   | Some _ ->
                       raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 "Pointer access requires pointer type")
                   | None ->
                       raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 ("Undefined variable in pointer access: " ^ name)))
              | _ ->
                  raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 "Complex lvalue pointer access not fully supported")
            in
            (match base_type with
             | TStruct (struct_name, fields) ->
                 (try List.assoc field_name fields
                  with Not_found ->
                    ...
                    | Some global_fields ->
                        (try List.assoc field_name global_fields
                         with Not_found -> raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 ("Struct field not found: " ^ field_name)))
                    | None -> raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 ("Struct field not found: " ^ field_name)))
             | _ -> raise (Error.type_error ~filename:None ~line:1 ~column:1 ~offset:0 "Pointer field access requires struct type"))
      in
      
      (* Validate assignment operation *)
      let result_type = match assign_op with
        | Assign -> 
            if normalize_type lvalue_type <> normalize_type rhs_type then
              type_error_at (get_expression_position expr) "Assignment type mismatch";
            lvalue_type
        | AddAssign | SubAssign | MulAssign | DivAssign | ModAssign ->
            ...
            if lvalue_norm <> rhs_norm then
              type_error_at (get_expression_position expr) "Compound assignment type mismatch";
            (match lvalue_norm with
             | TI32 | TF64 -> lvalue_type
             | _ -> type_error_at (get_expression_position expr) "Compound assignment requires numeric types")
        | BitAndAssign | BitOrAssign | BitXorAssign | ShlAssign | ShrAssign ->
            let lvalue_norm = normalize_type lvalue_type in
            let rhs_norm = normalize_type rhs_type in
            if lvalue_norm <> rhs_norm then
              type_error_at (get_expression_position expr) "Bitwise assignment type mismatch";
            (match lvalue_norm with
             | TI32 -> lvalue_type
             | _ -> type_error_at (get_expression_position expr) "Bitwise assignment requires integer types")
      in
      ignore result_type;
      env
  ...
  | GlobalVar (_, _, _, name, typ_opt, expr) ->
      ...
      (match typ_opt with
       | Some declared_type ->
           let expected_type = ast_type_to_simple_type declared_type in
           if normalize_type expr_type <> normalize_type expected_type then
             type_error_at (get_expression_position expr) "Global variable type mismatch";
           add_variable env name expected_type
       | None -> add_variable env name expr_type)
